...
//////////////////////////////////////////////////////////////////////
		...
		$intereslaboral= new clIlaboral();
          ...
          $retval  = '<script type="text/JavaScript" src="' . HOST .NAME. '/jquery/jquery.validate.js"></script>
            ...
            $retval .= '<form id="frmintereslaborals" action="'. INTERESLABORALS_URL.'intereslaborals.php" method="POST">';

           $Regresar = "regresar('". INTERESLABORALS_URL. "intereslaborals.php')";

            $retval .= '<fieldset class="fieldsetPequeno">';
            $retval .= '<legend class="etiquetaborde">
                            Interés Laboral<img src="'. IMAGENES_PATH .'/siguiente.png" style="border: 0px none;"> Listado Interés Laboral<img src="'. IMAGENES_PATH .'/siguiente.png" style="border: 0px none;"> '. $this->getStrEtiqueta() .'
                          </legend>
                       ';
            $NHC = "";		   
            $retval .= '
        ...
        public function getStrIngresar() {
            ...
            if($query->getStrSqlInsertUpdateDelete()){
                ...
                $ProcedimientoAlmacenado = sprintf("CALL spauditoria('%s', '%s', '%s', '%s');", $_SESSION["usuario"]["cuenta"], 'G', 'intereslaboral', $descripcion);
                $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
       ...
       public function getStrListar()
        {
            ...
            $retval .= '<legend class="etiquetaborde">
                            Forma de Interés Laboral<img src="'. IMAGENES_PATH .'/siguiente.png" style="border: 0px none;"> Listado Forma de Interés Laboral</legend>
                       ';
            $retval .= '
            ...
            if( count($resultado) > 0 )
            {
                ...
                foreach( $resultado as $rst):
                    ...
                    $retval .= 	'<td align="center"><div class="vtip" title="Actualizar <br> [ codigo = '.$rst["tilaboral_id"] .' ]">';
                    $retval .=  '<a href="'. INTERESLABORALS_URL .'intereslaborals.php?btnActualizar=Actualizar&strCodigo='. $rst["tilaboral_id"] .'"><img src="'. IMAGENES_PATH .'/actualizar.png" title="" width="16px" height="16px"  border="0" /></a>';
                    $retval .= 	'</div></td>';
                    $retval .= 	'<td align="center"><div class="vtip" title="Eliminar <br> [codigo = '.$rst["tilaboral_id"] .' ]">';
                    $retval .=  '<a href="'.INTERESLABORALS_URL .'intereslaborals.php?btnEliminar=Eliminar&strCodigo='. $rst["tilaboral_id"] .'" onclick="return confirmar()"><img src="'. IMAGENES_PATH .'/eliminar.png" title="" width="16px" height="16px"  border="0" /></a>';
                    $retval .= 	'</div></td>';
                    $retval .= 	'<td align="center"><div class="vtip" title="Detalle Interés Laboral <br> [ codigo = '.$rst["tilaboral_id"] .' ]">';
                    $retval .=  '<a href="'.INTERESLABORALS_URL.'intereslaborals.php?btnDetalle=Detalle&strCodigo='. $rst["tilaboral_id"] .'"><img src="'. IMAGENES_PATH .'/detalle.png" title="" width="16px" height="16px"  border="0" /></a>';
                    $retval .= 	'</div></td>';
                    $retval .= '</tr>';
                    $i = 1 - $i;
                endforeach;
            ...
            $paginacion->setStrNombrePagina("intereslaborals/intereslaborals.php");
 ...
 public function getStrEliminar() 
	...
	public function getStrActualizar() 
	{
            ...
            if($query->getStrSqlInsertUpdateDelete()){
                //Nombre Procedimientos Almacenados [Auditoria]
                $descripcion = 'Interes Laboral = [ '.$this->getStrIntereslaboral().' ] usuario = [ '. $this->getStrCodigo().' ] ';
                $ProcedimientoAlmacenado = sprintf("CALL spauditoria('%s', '%s', '%s', '%s');", $_SESSION["usuario"]["cuenta"], 'ACTUALIZARG', 'intereslaboral', $descripcion);
                $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
                $query->getStrSqlInsertUpdateDelete();
                $resultado = true;
 			}
                ...
                foreach( $resultado as $rst):
                    ...
                    $retval .= '<tr>
                                    <td colspan="2" align="center">
                                        <a href="'. INTERESLABORALS_URL.'intereslaborals.php?btnNuevo=Nuevo&strCodigo='.$v.'">| <img src="'. IMAGENES_PATH .'/atras.png" title="Regresar" width="16px" height="16px"  border="0" /> Regresar Listado Interés Laboral|</a>
                                    <td>
                                </tr>
                                ';
