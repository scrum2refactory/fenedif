...
//////////////////////////////////////////////////////////////////////////////
        ...
        public function getStrFormulario()
        {
          ...
          $retval  = '<script type="text/JavaScript" src="' . HOST .NAME. '/jquery/jquery.validate.js"></script>
            ...
            $retval .= '<form id="frmtasignacionpu" action="'. TASIGNACIONPU_URL .'tasignacionpu.php" method="POST">';

            $Regresar = "regresar('". TEMPRESA_URL . "tempresa.php')";

            $retval .= '<fieldset class="fieldsetPequeno">';
            $retval .= '<legend class="etiquetaborde">
                            ASIGNACIÓN PUESTO <img src="'. IMAGENES_PATH .'/siguiente.png" style="border: 0px none;">LISTADO ASIGNACIÓN PUESTO<img src="'. IMAGENES_PATH .'/siguiente.png" style="border: 0px none;"> '. $this->getStrEtiqueta() .'
                          </legend>
                       ';
            $NHC = "";		   
            $retval .= '
        ...
        public function getStrIngresar() 
        {
           ...
           //Nombre Procedimientos Almacenados
              foreach($this->getStrusuarioscu() as $folio20)
              {
              	$query = new clQuery();
            	$resultado = false; 
	            $ProcedimientoAlmacenado = sprintf("CALL spingresartusuariopu('%s', '%s');",
...
public function getStrEliminar() 
        {
           $query = new clQuery();
            $resultado = TRUE;
            //Nombre Procedimientos Almacenados
            $ProcedimientoAlmacenado = sprintf("CALL speliminarusuariopuesto('%s');", $this->getStrCodigo());
            $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
	...
	public function getStrBuscarapemp($v) {
            $query = new clQuery();
            $resultado = false;
            //Nombre Procedimientos Almacenados
            $cf=$_SESSION["usuario"]["suc"];
			$me=$_SESSION["usuario"]["cuenta"];
            $ProcedimientoAlmacenado = sprintf("CALL spbidentificadorapemp('%d');","$v");
			$query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
 	...
 	$query = new clQuery();
            $resultado = false;
            //Nombre Procedimientos Almacenados
            $ProcedimientoAlmacenado = sprintf("CALL spbusuariopuesto('%s');", $this->getStrCodigo());
            $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
            $resultado = $query->getStrSqlSelect();
            ...
            {
                foreach( $resultado as $rst):   
	                $this->setStrCodigo($rst['id_usuario_puesto']);
    ...
    public function getStrListar()
        ...
        $paginacion = new clPaginacion();
            ...
            //Nombre Procedimientos Almacenados
            $ProcedimientoAlmacenado = sprintf("CALL sptotalusuariopu();", $this->getStrCodigo());
            $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
            ...
            $ProcedimientoAlmacenado = sprintf("CALL splistarusuariopuesto('%d','%d','%d');","$re", "$pa","$cf");
            $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
            $resultado = $query->getStrSqlSelect();

            $retval .= '<fieldset class="fieldsetGrande">';
            $retval .= '<legend class="etiquetaborde">
                            ASIGNACIÓN PUESTO<img src="'. IMAGENES_PATH .'/siguiente.png" style="border: 0px none;">LISTADO ASIGNACIÓN PUESTO </legend>
                       ';
            $retval .= '
                        <table border="0" width="100%" cellpadding="1" cellspacing="1" align="center">
                            <tr class="tablatitulo">
                                <th colspan="11">LISTADO USUARIOS PUESTO</th>
                            </tr>
                            <tr class="tablasubtitulo">
                            	<th align="center"></th>  
                               <th align="center">ID Usuario Curso</th>                                                              
            ...
            if( count($resultado) > 0 )
            {
                ...
                foreach( $resultado as $rst):
					...
					$retval .= 	'<td align="center"><div class="vtip" title="Eliminar <br> [codigo = '.$rst["id_usuario_puesto"] .' ]">';
                    $retval .=  '<a href="'.TASIGNACIONPU_URL.'tasignacionpu.php?btnEliminar=Eliminar&strCodigo='. $rst["id_usuario_puesto"] .'" onclick="return confirmar()"><img src="'. IMAGENES_PATH .'/eliminar.png" title="" width="16px" height="16px"  border="0" /></a>';
                    $retval .= 	'</div></td>';
                    $retval .= 	'<td align="center"><div class="vtip" title="Detalle Usuarios Asignados <br> [ codigo = '.$rst["id_usuario_puesto"] .' ]">';
                    $retval .=  '<a href="'.TASIGNACIONPU_URL.'tasignacionpu.php?btnDetalle=Detalle&strCodigo='. $rst["id_usuario_puesto"] .'"><img src="'. IMAGENES_PATH .'/detalle.png" title="" width="16px" height="16px"  border="0" /></a>';
                    $retval .= 	'</div></td>';
                    $retval .= '</tr>';
                    $i = 1 - $i;
                endforeach;
            ...
            $paginacion->setStrNombrePagina("tasignacionpu/tasignacionpu.php");
...
public function getStrListarb()
        {
           $retval  = '<script type="text/JavaScript" src="' . HOST .NAME. '/jquery/jquery.validate.js"></script>
            ...
            $retval .= '<br><br><br><br>
                       ...
                       ';	
			$retval .= '<form id="frmtasignacionpub" action="'. TASIGNACIONPU_URL .'tasignacionpu.php" method="POST">';			   
            $NHC = "";		   
		    $Regresar = "regresar('". TEMPRESA_URL. "tempresa.php')";
		    $query = new clQuery();
            //Nombre Procedimientos Almacenados
			...
			$ProcedimientoAlmacenado = sprintf("CALL splistarusuariocu('%s','%s','%s');","$fi","$ff","$sucursal");
            $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
            $resultado = $query->getStrSqlSelect();
...
public function getStrDetallePersona()
        {
        	$query = new clQuery();
            //Nombre Procedimientos Almacenados
            $ProcedimientoAlmacenado = sprintf("CALL spbdetalleusuariopuesto('%s');", $this->getStrCodigo());
            $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
            $resultado = $query->getStrSqlSelect();
            ...
            if( count($resultado) > 0 )
            {
				...
				$valor=$this->getStrCodigo();
			 	$v=$this->getStrBuscarapemp($valor);
                foreach( $resultado as $rst):
                    $retval .= '<table border="0" width="100%" cellpadding="2" cellspacing="2" align="center">';
                    $retval .= '<tr>
                                    <td colspan="2" align="center">
                                        <a href="'. TASIGNACIONPU_URL .'tasignacionpu.php?btnNuevo=Nuevo&strCodigo='.$v.'">| <img src="'. IMAGENES_PATH .'/atras.png" title="Regresar" width="16px" height="16px"  border="0" /> Regresar Listado de Empresas|</a>
                                    <td>
                                </tr>
                                ';
                    ...
                    $retval .= '<tr class="listadofila0">
                                    ...
                                    <td align="left">  '. $rst["id_usuario_puesto"] .'</td>
                                </tr>
                                ';
                    $retval .= '<tr class="listadofila0">
