...
////////////////////////////////////////////////////////////////////////	
        ...
        public function getStrFormulario()
        {
          ...
          $retval  = '<script type="text/JavaScript" src="' . HOST .NAME. '/jquery/jquery.validate.js"></script>
            ...
            $retval .= '<form id="frmthorarioemp" action="'. THORARIOEMP_URL.'thorarioemp.php" method="POST">';

            $Regresar = "regresar('". CFORMATIVO_URL . "cformativo.php')";

            $retval .= '<fieldset class="fieldsetPequeno">';
            $retval .= '<legend class="etiquetaborde">
                            HORARIO LABORAL <img src="'. IMAGENES_PATH .'/siguiente.png" style="border: 0px none;">LISTADO HORARIO LABORAL<img src="'. IMAGENES_PATH .'/siguiente.png" style="border: 0px none;"> '. $this->getStrEtiqueta() .'
                          </legend>
                       ';
            $NHC = "";		   
            $retval .= '
                        <table width="100%" border="0" align="center" cellpadding="1">
							...
							<table width="100%" border="0" align="center" cellpadding="1">
                            <tr>
                                <td colspan="4" align="center" class="tablatitulo">SELECCIONE HORARIO LABORAL</td>
                            </tr>
		                  	<tr class="formulariofila1">
		                  	   <td align="left">
        ...
        public function getStrIngresar() 
        {
            //Nombre Procedimientos Almacenados
              foreach($this->getStrDias() as $folio20)
              {
              	$query = new clQuery();
            	$resultado = false; 
	            $ProcedimientoAlmacenado = sprintf("CALL spingresarhorarioemp('%s', '%s','%s', '%s');",
	            ...
	            if($query->getStrSqlInsertUpdateDelete())
	            {
	                ...
	                $ProcedimientoAlmacenado = sprintf("CALL spauditoria('%s', '%s', '%s', '%s');", $_SESSION["usuario"]["cuenta"], 'GUARDAR', 'thorario_empresa', $descripcion);
	                $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
	...
	public function getStrActualizar() 
		{
               ...
               //Nombre Procedimientos Almacenados
              	...
              	$query = new clQuery();
            	$resultado = false; 
	            $ProcedimientoAlmacenado = sprintf("CALL spactualizarhorarioemp('%s', '%s','%s', '%s');",
	            ...
	            if($query->getStrSqlInsertUpdateDelete())
	            {
	                //Nombre Procedimientos Almacenados [Auditoria]
	                $descripcion = 'Dias = [ '.$this->getStrDias().' ]  ]';
	                $ProcedimientoAlmacenado = sprintf("CALL spauditoria('%s', '%s', '%s', '%s');", $_SESSION["usuario"]["cuenta"], 'ACTUALIZAR', 'thorario_empresa', $descripcion);
	                $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
	                $query->getStrSqlInsertUpdateDelete();
	                $resultado = true;
	            }
 ...
 public function getStrEliminar() 
        {
            ...
            $resultado = TRUE;
            //Nombre Procedimientos Almacenados
            $ProcedimientoAlmacenado = sprintf("CALL speliminarthorarioemp('%s');", $this->getStrCodigo());
            $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
			...
			if($query->getStrSqlInsertUpdateDelete())
	            {
	                ...
	                $ProcedimientoAlmacenado = sprintf("CALL spauditoria('%s', '%s', '%s', '%s');", $_SESSION["usuario"]["cuenta"], 'ELIMINAR', 'thorario_empresa', $descripcion);
	                $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
	                $query->getStrSqlInsertUpdateDelete();
	                $resultado = true;
	            }
	...
	public function getStrBuscarthemp($v) 
	{
            $query = new clQuery();
            $resultado = false;
            //Nombre Procedimientos Almacenados
            $ProcedimientoAlmacenado = sprintf("CALL spbidentificadorthemp('%d');","$v");
			$query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
 	...
 	public function getStrBuscar() 
 	{
 		$query = new clQuery();
            $resultado = false;
            //Nombre Procedimientos Almacenados
            $ProcedimientoAlmacenado = sprintf("CALL spbthorarioemp('%s');", $this->getStrCodigo());
            $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
            $resultado = $query->getStrSqlSelect();
            ...
            {
                foreach( $resultado as $rst):   
	                $this->setStrCodigo($rst['horario_id']);
    ...
    public function getStrListar()
        {
            ...
            //Nombre Procedimientos Almacenados
            $ProcedimientoAlmacenado = sprintf("CALL sptotalhorarioemp();", $this->getStrCodigo());
            $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
            ...
            $ProcedimientoAlmacenado = sprintf("CALL splistarhorarioemp('%d','%d','%d');","$re", "$pa","$cf");
            $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
            $resultado = $query->getStrSqlSelect();

            $retval .= '<fieldset class="fieldsetGrande">';
            $retval .= '<legend class="etiquetaborde">
                            HORARIO LABORAL<img src="'. IMAGENES_PATH .'/siguiente.png" style="border: 0px none;">LISTADO HORARIO LABORAL</legend>
                       ';
            $retval .= '
                        <table border="0" width="100%" cellpadding="1" cellspacing="1" align="center">
                            <tr class="tablatitulo">
                                <th colspan="11">LISTADO DÍAS LABORALES</th>
                            </tr>
            ...
            if( count($resultado) > 0 )
            {
                ...
                foreach( $resultado as $rst):
					...
					$retval .= 	'<td align="center"><div class="vtip" title="ACTUALIZAR HORARIO LABORAL <br> [ codigo = '.$rst["horario_id"] .' ]">';
                    $retval .=  '<a href="'.THORARIOEMP_URL.'thorarioemp.php?btnActualizar=Actualizar&strCodigo='. $rst["horario_id"] .'"><img src="'. IMAGENES_PATH .'/actualizar.png" title="" width="16px" height="16px"  border="0" /></a>';
                    $retval .= 	'</div></td>';
                    $retval .= 	'<td align="center"><div class="vtip" title="ELIMINAR HORARIO LABORAL <br> [codigo = '.$rst["horario_id"] .' ]">';
                    $retval .=  '<a href="'.THORARIOEMP_URL.'thorarioemp.php?btnEliminar=Eliminar&strCodigo='. $rst["horario_id"] .'" onclick="return confirmar()"><img src="'. IMAGENES_PATH .'/eliminar.png" title="" width="16px" height="16px"  border="0" /></a>';
                    $retval .= 	'</div></td>';
                    $retval .= 	'<td align="center"><div class="vtip" title="DETGALLE HORARIO LABORAL<br> [ codigo = '.$rst["horario_id"] .' ]">';
                    $retval .=  '<a href="'.THORARIOEMP_URL.'thorarioemp.php?btnDetalle=Detalle&strCodigo='. $rst["horario_id"] .'"><img src="'. IMAGENES_PATH .'/detalle.png" title="" width="16px" height="16px"  border="0" /></a>';
                    $retval .= 	'</div></td>';
                    $retval .= '</tr>';
                    $i = 1 - $i;
                endforeach;
            ...
            $paginacion->setStrNombrePagina("thorarioemp/thorarioemp.php");
...
public function getStrDetallePersona()
        {
        	$query = new clQuery();
            //Nombre Procedimientos Almacenados
            $ProcedimientoAlmacenado = sprintf("CALL spbdetallethorarioemp('%s');", $this->getStrCodigo());
            $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
            $resultado = $query->getStrSqlSelect();

            $retval .= '<fieldset class="fieldsetGrande">';
            $retval .= '<legend class="etiquetaborde">
                           HORARIO LABORAL<img src="'. IMAGENES_PATH .'/siguiente.png" style="border: 0px none;">LISTADO HORARIO LABORAL<img src="'. IMAGENES_PATH .'/siguiente.png" style="border: 0px none;">DETALLE HORARIO LABORAL
                        </legend>
                       ';
            ...
            if( count($resultado) > 0 )
            {
				...
				$valor=$this->getStrCodigo();
			 	$v=$this->getStrBuscarthemp($valor);
                foreach( $resultado as $rst):
                    $retval .= '<table border="0" width="100%" cellpadding="2" cellspacing="2" align="center">';
                    $retval .= '<tr>
                                    <td colspan="2" align="center">
                                        <a href="'. THORARIOEMP_URL.'thorarioemp.php?btnNuevo=Nuevo&strCodigo='.$v.'">| <img src="'. IMAGENES_PATH .'/atras.png" title="Regresar" width="16px" height="16px"  border="0" />REGRESAR LISTADO HORARIO LABORAL|</a>
                                    <td>
                                </tr>
                                ';
                    
                    $retval .= '<tr class="tablatitulo">
                                    <th colspan="2">DETALLE&nbsp; HORARIO LABORAL;REGISTRADO</th>
                                </tr>
                                ';
                    ...
                    $retval .= '<tr class="listadofila0">
                                ...
                                ';
                    $retval .= '<tr class="listadofila0">
                                    <th align="right" class="bienvenida">Día:</th>
                                    <td align="left">  '. $rst["dia_nombre"] .'</td>
                                </tr>
                                ';     	           				 					 					
					$retval .= '<tr class="listadofila0">
                                    <th align="right" class="bienvenida">Hora inicio:</th>
                                    <td align="left">  '. $rst["hinicio"] .'</td>
                                </tr>
                                '; 
					$retval .= '<tr class="listadofila0">
                                    <th align="right" class="bienvenida">Hora fin:</th>
                                    <td align="left">  '. $rst["hfin"] .'</td>
                                </tr>
                                '; 									
                    $i = 1 - $i;
                endforeach;
