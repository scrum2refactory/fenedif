...
/////////////////////////////////////////////////////////////////////////////
		...
		$ilaboral = new clIlaboral();
          $retval  = '<script type="text/JavaScript" src="' . HOST .NAME. '/jquery/jquery.validate.js"></script>
            ...
            $retval .= '<form id="frmdisponibilidadl" action="'. DISPONIBILIDADL_URL.'disponibilidadl.php" method="POST">';

            $Regresar = "regresar('". DISPONIBILIDADL_URL. "disponibilidadl.php')";
            $retval .= '<fieldset class="fieldsetPequeno">';
            $retval .= '<legend class="etiquetaborde">
                            DISPONIBILIDAD LABORAL<img src="'. IMAGENES_PATH .'/siguiente.png" style="border: 0px none;">LISTADO DISPONIBILIDAD LABORAL<img src="'. IMAGENES_PATH .'/siguiente.png" style="border: 0px none;"> '. $this->getStrEtiqueta() .'
                          </legend>
                       ';
            $NHC = "";		   
            $retval .= '
      ...
      public function getStrIngresar() 
        {
		 ...
		 if(count($First)>0)
		 {
            ...
            if($query->getStrSqlInsertUpdateDelete())
                ...
                $ProcedimientoAlmacenado = sprintf("CALL spauditoria('%s', '%s', '%s', '%s');", $_SESSION["usuario"]["cuenta"], 'GUARDAR', 'tdlaboraldias', $descripcion);
                $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
            ...
            if($query->getStrSqlInsertUpdateDelete()){
                ...
                $ProcedimientoAlmacenado = sprintf("CALL spauditoria('%s', '%s', '%s', '%s');", $_SESSION["usuario"]["cuenta"], 'GUARDAR', 'tdlaboral', $descripcion);
                $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
                $query->getStrSqlInsertUpdateDelete();
                $resultado = true;
           	}
...
public function getStrActualizar() 
	{
            ...
            if($query->getStrSqlInsertUpdateDelete()){
                ...
                $ProcedimientoAlmacenado = sprintf("CALL spauditoria('%s', '%s', '%s', '%s');", $_SESSION["usuario"]["cuenta"], 'ACTUALIZAR', 'tdlaboral', $descripcion);
                $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
                $query->getStrSqlInsertUpdateDelete();
                $resultado = true;
            }
...
public function getStrEliminar() 
        {
 		 ...
 		 if($query->getStrSqlInsertUpdateDelete()){
                ...
                $ProcedimientoAlmacenado = sprintf("CALL spauditoria('%s', '%s', '%s', '%s');", $_SESSION["usuario"]["cuenta"], 'ELIMINAR', 'tdlaboral', $descripcion);
                $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
                $query->getStrSqlInsertUpdateDelete();
                $resultado = true;
            }
...
public function getStrEliminardias() 
        {
 		 ...
 		 if($query->getStrSqlInsertUpdateDelete()){
                //Nombre Procedimientos Almacenados [Auditoria]
                $descripcion = 'dias = [ '.$this->getStrDias().' ] codigo = [ '. $this->getStrCodigo().' ] ';
                $ProcedimientoAlmacenado = sprintf("CALL spauditoria('%s', '%s', '%s', '%s');", $_SESSION["usuario"]["cuenta"], 'ELIMINAR', 'tdlaboraldias', $descripcion);
                $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
                $query->getStrSqlInsertUpdateDelete();
                $resultado = true;
            }
  ...
  public function getStrListar()
        {
            ...
            $retval .= '<legend class="etiquetaborde">
                            DISPONIBILIDAD LABORAL<img src="'. IMAGENES_PATH .'/siguiente.png" style="border: 0px none;">LISTADO DISPONIBILIDAD LABORAL</legend>
                       ';
            $retval .= '
            ...
            if( count($resultado) > 0 )
            {
                ...
                foreach( $resultado as $rst):
                    ...
                    $retval .=  '<a href="'.DISPONIBILIDADL_URL.'disponibilidadl.php?btnActualizar=Actualizar&strCodigo='. $rst["dlaboral_id"] .'"><img src="'. IMAGENES_PATH .'/actualizar.png" title="" width="16px" height="16px"  border="0" /></a>';
                    $retval .= 	'</div></td>';
                    $retval .= 	'<td align="center"><div class="vtip" title="Eliminar <br> [codigo = '.$rst["dlaboral_id"] .' ]">';
                    $retval .=  '<a href="'. DISPONIBILIDADL_URL .'disponibilidadl.php?btnEliminar=Eliminar&strCodigo='. $rst["dlaboral_id"] .'" onclick="return confirmar()"><img src="'. IMAGENES_PATH .'/eliminar.png" title="" width="16px" height="16px"  border="0" /></a>';
                    $retval .= 	'</div></td>';
                    $retval .= 	'<td align="center"><div class="vtip" title="Detalle Centro Formativo <br> [ codigo = '.$rst["dlaboral_id"] .' ]">';
                    $retval .=  '<a href="'. DISPONIBILIDADL_URL .'disponibilidadl.php?btnDetalle=Detalle&strCodigo='. $rst["dlaboral_id"] .'"><img src="'. IMAGENES_PATH .'/detalle.png" title="" width="16px" height="16px"  border="0" /></a>';
                    $retval .= 	'</div></td>';
                    $retval .= '</tr>';
                    $i = 1 - $i;
                endforeach;
            ...
            $paginacion->setStrNombrePagina("disponibilidadl/disponibilidadl.php");
  ...
  public function getStrListardias()
        {
            ...
            if( count($resultado) > 0 )
            {
                ...
                foreach( $resultado as $rst):
                    ...
                    $retval .=  '<a href="'. DISPONIBILIDADL_URL .'disponibilidadl.php?btnEliminar=Eliminardias&strCodigo='. $rst["tdlaboraldias_id"] .'" onclick="return confirmar()"><img src="'. IMAGENES_PATH .'/eliminar.png" title="" width="16px" height="16px"  border="0" /></a>';
                    $retval .= 	'</div></td>';
...
public function getStrDetallePersona()
        {
            ...
            $retval .= '<fieldset class="fieldsetGrande">';
            $retval .= '<legend class="etiquetaborde">
                            DISPONIBILIDAD LABORAL<img src="'. IMAGENES_PATH .'/siguiente.png" style="border: 0px none;">LISTADO DISPONIBILIDAD LABORAL<img src="'. IMAGENES_PATH .'/siguiente.png" style="border: 0px none;">DETALLE DISPONIBILIDAD LABORAL
                        </legend>
                       ';
                ...
                foreach( $resultado as $rst):
                    ...
                    $retval .= '<tr>
                                    <td colspan="2" align="center">
                                        <a href="'.DISPONIBILIDADL_URL.'disponibilidadl.php?btnNuevo=Nuevo&strCodigo='.$v.'">| <img src="'. IMAGENES_PATH .'/atras.png" title="Regresar" width="16px" height="16px"  border="0" />REGRESAR  DISPONIBILIDAD LABORAL|</a>
                                    <td>
                                </tr>
                                ';
                    
                    $retval .= '<tr class="tablatitulo">
                                    <th colspan="2">DETALLE&nbsp;FORMAS DE DISPONIBILIDAD LABORAL;REGISTRADO</th>
                                </tr>
                                ';
