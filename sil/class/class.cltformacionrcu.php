        ...
        public function getStrFormulario()
        {
          ...
          $retval  = '<script type="text/JavaScript" src="' . HOST .NAME. '/jquery/jquery.validate.js"></script>
            ...
            $retval .= '<br><br><br><br>
                        <script>
                            $(document).ready(function(){
                                    ...
                                    $("#lsTfnecesaria option:selected").each(function () {
                                            ...
                                            $.post( "'. TFORMACIONRCU_URL .'tformacionrcu.php", { btnBuscar: "nivel",
								 ...
								 $("#lsTfnecesaria").change(function () {
                                    $("#lsTfnecesaria option:selected").each(function () {
                                            var tfnecesaria = $(this).val();
                                            $.post( "'. TFORMACIONRCU_URL .'tformacionrcu.php", { btnBuscar: "niveledu",
                                                    strCodigotfnecesaria: tfnecesaria
                                                                                    },
                                        function(data){
                       ...
                       ';
            $retval .= '<form id="frmtformacionrcu" action="'. TFORMACIONRCU_URL .'tformacionrcu.php" method="POST">';
            ...
            $retval .= '
                        <table width="100%" border="0" align="center" cellpadding="1">
							...
							<tr class="formulariofila1">
                                <td align="right"><b>Formacion Necesaria:&nbsp;</b></td>
                                <td align="left">                                    
  ...
  public function getStrIngresar() 
		...
		$Second = $this->getstrCminimos();
		 if(count($First)>0)
		 {
            ...
            $ProcedimientoAlmacenado = sprintf("CALL spingresartformacionrcu('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s');",
	...
	public function getStrActualizar() 
	{
            //Nombre Procedimientos Almacenados
              	...
              	$query = new clQuery();
            	$resultado = false; 
	            $ProcedimientoAlmacenado = sprintf("CALL spactualizartformacionrcu('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s');",
            ...
            if($query->getStrSqlInsertUpdateDelete())
	            {
	                //Nombre Procedimientos Almacenados [Auditoria]
	                $descripcion = 'F_Necesaria = [ '.$this->getStrTfnecesaria().' ]  ]';
	                $ProcedimientoAlmacenado = sprintf("CALL spauditoria('%s', '%s', '%s', '%s');", $_SESSION["usuario"]["cuenta"], 'ACTUALIZAR', 'tformacionrcu', $descripcion);
	                $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
	                $query->getStrSqlInsertUpdateDelete();
	                $resultado = true;
	            }
 ...
 public function getStrEliminar() 
        {
           ...
           $resultado = TRUE;
            //Nombre Procedimientos Almacenados
            $ProcedimientoAlmacenado = sprintf("CALL speliminartformacionrcu('%s');", $this->getStrCodigo());
            $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
			...
			if($query->getStrSqlInsertUpdateDelete())
	            {
	                //Nombre Procedimientos Almacenados [Auditoria]
	                $descripcion = 'F_Necesaria = [ '.$this->getStrTfnecesaria().' ]  ]';
	                $ProcedimientoAlmacenado = sprintf("CALL spauditoria('%s', '%s', '%s', '%s');", $_SESSION["usuario"]["cuenta"], 'ELIMINAR', 'tformacionrcu', $descripcion);
	                $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
	                $query->getStrSqlInsertUpdateDelete();
	                $resultado = true;
	            }
	...
	public function getStrBuscartfcu($v) {
            $query = new clQuery();
            $resultado = false;
            //Nombre Procedimientos Almacenados
            $cf=$_SESSION["usuario"]["suc"];
			$me=$_SESSION["usuario"]["cuenta"];
            $ProcedimientoAlmacenado = sprintf("CALL spbidentificadortfcu('%d');","$v");
			$query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
 	...
 	public function getStrBuscar() 
 	{
          $query = new clQuery();
            $resultado = false;
            //Nombre Procedimientos Almacenados
            $ProcedimientoAlmacenado = sprintf("CALL spbtformacionrcu('%s');", $this->getStrCodigo());
            $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
            $resultado = $query->getStrSqlSelect();
            ...
            {
                foreach( $resultado as $rst):   
	                $this->setStrCodigo($rst['tformacionrcu_id']);
 ...
 public function getStrListar()
        {
            ...
            //Nombre Procedimientos Almacenados
            $ProcedimientoAlmacenado = sprintf("CALL sptotaltformacionrcu();", $this->getStrCodigo());
            $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
            ...
            $ProcedimientoAlmacenado = sprintf("CALL splistartformacionrcu('%d','%d','%d');","$re", "$pa","$cf");
            $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
            $resultado = $query->getStrSqlSelect();
            ...
            if( count($resultado) > 0 )
            {
                ...
                foreach( $resultado as $rst):
                    ...
                    $retval .= 	'<td align="center">'. $rst["tformacionrcu_id"] .'</td>';
					...
					$retval .= 	'<td align="center"><div class="vtip" title="Actualizar <br> [ codigo = '.$rst["tformacionrcu_id"] .' ]">';
                    $retval .=  '<a href="'.TFORMACIONRCU_URL.'tformacionrcu.php?btnActualizar=Actualizar&strCodigo='. $rst["tformacionrcu_id"] .'"><img src="'. IMAGENES_PATH .'/actualizar.png" title="" width="16px" height="16px"  border="0" /></a>';
                    $retval .= 	'</div></td>';
                    $retval .= 	'<td align="center"><div class="vtip" title="Eliminar <br> [codigo = '.$rst["tformacionrcu_id"] .' ]">';
                    $retval .=  '<a href="'.TFORMACIONRCU_URL.'tformacionrcu.php?btnEliminar=Eliminar&strCodigo='. $rst["tformacionrcu_id"] .'" onclick="return confirmar()"><img src="'. IMAGENES_PATH .'/eliminar.png" title="" width="16px" height="16px"  border="0" /></a>';
                    $retval .= 	'</div></td>';
                    $retval .= 	'<td align="center"><div class="vtip" title="Detalle Cursos <br> [ codigo = '.$rst["tformacionrcu_id"] .' ]">';
                    $retval .=  '<a href="'.TFORMACIONRCU_URL.'tformacionrcu.php?btnDetalle=Detalle&strCodigo='. $rst["tformacionrcu_id"] .'"><img src="'. IMAGENES_PATH .'/detalle.png" title="" width="16px" height="16px"  border="0" /></a>';
                    $retval .= 	'</div></td>';
                    $retval .= '</tr>';
                    $i = 1 - $i;
                endforeach;
...
public function getStrDetallePersona()
        {
        	$query = new clQuery();
            //Nombre Procedimientos Almacenados
            $ProcedimientoAlmacenado = sprintf("CALL spbdetalletformacionrcu('%s');", $this->getStrCodigo());
            $query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
            $resultado = $query->getStrSqlSelect();

            $retval .= '<fieldset class="fieldsetGrande">';
            $retval .= '<legend class="etiquetaborde">
                           FORMACIÓN REQUERIDA <img src="'. IMAGENES_PATH .'/siguiente.png" style="border: 0px none;">LISTADO FORMACIÓN REQUERIDA  <img src="'. IMAGENES_PATH .'/siguiente.png" style="border: 0px none;">DETALLE FORMACIÓN REQUERIDA 
                        </legend>
                       ';
            ...
            if( count($resultado) > 0 )
            {
				...
				$valor=$this->getStrCodigo();
			 	$v=$this->getStrBuscartfcu($valor);
                foreach( $resultado as $rst):
                    $retval .= '<table border="0" width="100%" cellpadding="2" cellspacing="2" align="center">';
                    $retval .= '<tr>
                                    <td colspan="2" align="center">
                                        <a href="'. TFORMACIONRCU_URL .'tformacionrcu.php?btnNuevo=Nuevo&strCodigo='.$v.'">| <img src="'. IMAGENES_PATH .'/atras.png" title="Regresar" width="16px" height="16px"  border="0" />REGRESAR LISTADO FORMACIÓN REQUERIDA |</a>
                                    <td>
                                </tr>
                                ';
                    ...
                    $retval .= '<tr class="listadofila0">
                                    ...
                                    <td align="left">  '. $rst["tformacionrcu_id"] .'</td>
                                </tr>
                                ';
                    $retval .= '<tr class="listadofila0">
					...
					$retval .= '<tr class="listadofila0">
                                ...
                                '; 	
					$retval .= '<tr class="listadofila0">
                                    <th align="right" class="bienvenida">NOMBRE CURSO:</th>
