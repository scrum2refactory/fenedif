...
//////////////////////////valor a////////////////////////////////////
 ...
 public function getStrEliminar() 
	...
	public function getStrBuscartscf($v) {
            $query = new clQuery();
            $resultado = false;
            //Nombre Procedimientos Almacenados
            $cf=$_SESSION["usuario"]["suc"];
			$me=$_SESSION["usuario"]["cuenta"];
            $ProcedimientoAlmacenado = sprintf("CALL spbidentificadortscf('%d');","$v");
			$query->setStrProcedimientoAlmacenado($ProcedimientoAlmacenado);
...
public function getStrDetallePersona()
        {
            ...
            if( count($resultado) > 0 )
            {
				...
				$valor=$this->getStrCodigo();
			 	$v=$this->getStrBuscartscf($valor);
                foreach( $resultado as $rst):
                    $retval .= '<table border="0" width="100%" cellpadding="2" cellspacing="2" align="center">';
                    $retval .= '<tr>
                                    <td colspan="2" align="center">
                                        <a href="'. TSEGUIMIENTOCF_URL .'tseguimientocf.php?btnNuevo=Nuevo&strCodigo='.$v.'">| <img src="'. IMAGENES_PATH .'/atras.png" title="Regresar" width="16px" height="16px"  border="0" /> REGRESAR SEGUIMIENTO|</a>
                                    <td>
                                </tr>
                                ';
