...
INNER JOIN
			sucursal on tempresa.id_sucursal=sucursal.sucursal_id
where 
			tempresa.id_sucursal='$sucursal' and tseguimientoemp.fecha >='$ini' and  tseguimientoemp.fecha <='$fin' "; //Cuento el total de registros
...
INNER JOIN
			sucursal on tempresa.id_sucursal=sucursal.sucursal_id
where 
			tseguimientoemp.fecha >='$ini1' and  tseguimientoemp.fecha <='$ffin' "; //Cuento el total de registros
				$result = mysql_query($sSQL);
				$row = mysql_fetch_array($result);
				$totalRegistros = $row["count(*)"]; //Almaceno el total en una variable
...
{
	echo '<td><p class="centrado">
	<a target="_blank" href="../pdf/list_empresa_visitadas_todos.php?p1='.$sucursal_id.'&p2='.$estado.'
	&p3='.$ini.'
	&p4='.$fin.'
	" title="'.$id_excel.'"><img src="../imgs/excel_img.png" alt="'.$id_excel.'" /></a>
