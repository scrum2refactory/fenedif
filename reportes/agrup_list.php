...
$sel_act = mysql_query("SELECT * FROM notas WHERE agrupamiento='$sesion'");
$num_act = mysql_num_rows($sel_act);
if($num_act == 0)
	{
	echo '<p class="negrita">'.$id_notas.'</p>';
	}
else
	{
	echo '<p class="negrita">Notas registradas hasta el momento</p>';
	echo '<table class="tablacentrada">';
	echo '<tr class="encab">';
	echo '<td>'.$id_eliminar.'</td>
	<td>'.$id_activ.'</td>
	<td>Nota</td>
	<td>Descripcion</td>';
	...
	for($b=0;$b<$num_act;$b++)
		...
		echo '<td class="centrado"><a href="#" title="'.$id_eliminar.'" onclick="notaElimina(\''.$reg_act['id'].'\')"><img src="imgs/borra_peq.png" alt="'.$id_eliminar.'" /></a></td>';
		//echo '<td><a href="#" onclick="activSolicitaEdita(\''.$reg_act['id'].'\')" title="'.$id_editar.'">
	...
	}//fin de else (sí que hay actividad
