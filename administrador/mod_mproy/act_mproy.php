...
<br />
<br /><br /><br />
<font color=#000080><CENTER><H10>LISTA DE PROYECTOS ASIGNADOS</H10></div></CENTER></font>
<?php
$usuario_activo=$_SESSION["login"];
$sel_agr = mysql_query("select * from agrupamientos where docente = '$usuario_activo'");
$num_agr = mysql_num_rows($sel_agr);
if($num_agr>0)
{
	echo '<table width="500" align="center"  class="tabla">';
	for($agr=0;$agr<$num_agr;$agr++)
			{
			echo '<tr>';	
			$reg_agr = mysql_fetch_array($sel_agr);
			//echo '<li><a href=../act_mproy.php?agrupamiento='.$reg_agr['agrupamiento'].'\')">'.$reg_agr['nombre'].'</a></td></li>';
			echo "<td class=dtabla align=center><a href=\"act_acti.php?p4=not&p1=".$reg_agr["agrupamiento"]."\">".$reg_agr['nombre']."</a></td>";
			echo '</tr>';
			}
		echo '</table>	';	
}
...
{
	cuadro_error(" NO TIENE PROYECTOS ASIGNADOS <b><a href=act_mproy.php  target=\"_self\">    Ir a Registrar</a></b>");	
}
...
$id_agr=$_REQUEST["agrupamiento"];
$sesion=$_REQUEST["agrupamiento"];
echo '<br/><span class="negrita"><p class="centrado">PROYECTO: '.$sesion.'. '.$id_fecha.': '.$nombre_dia.', '.$fecha_esp.'</p></span>';
...
echo '</p><br />';
echo '<p class="centrado">';
//echo '<a href=\"act_mproy.php?accion=nota&docente='.$sesion.'\')"><font color=#000080>CALIFICACIONES</font></a>&nbsp;';
echo '<a href="#" onclick="abreAgrup1(\''.$sesion.'\')"><font color=#000080>OBSERVACIONES</font></a>&nbsp;';
echo '</p><br />';
//<td class=\"tdatos\"><a href=\"bus_coord.php?busqueda=docente&docente=".$row["docente"]."\">".$row["docente"]."</a></td>
/////////////////////////////fin de  vance/////////////////////////////////////////////
$accion1 = $_POST['p4'];
...
//<form name="registro" action="act_coord.php" method="post" enctype="multipart/form-data">	
echo '<form id="notas" action="act_mproy.php" name="notas" method="post" enctype="multipart/form-data">';
...
echo '<option value="0">elige actividad</option>';
...
echo '</tr>';
echo '</table><br />';
echo '<p class=centrado><a href='."'".'act_mproy.php?registro=not&agrupamiento='.$sesion.'&numero='.$num_alum.'&actividad='.$act.'&descripcion='.$descripcion.'\'"><img src="../imgs/guardar.png" alt="'.$id_regnotas.'" /></a></p>';
//<input type="button" value="Actualizar Datos" onclick="location.href='."'".'act_coord.php?docente=docente&docente='.$docente."'".'">
//<td colspan="2" align="center"><input type="submit" name="acc" value="Guardar"> 
//<input type="button" value="Eliminar" onclick="location.href='."'".'elim_coord.php?docente=docente&docente='.$docente."'".'">
//<td class=\"tdatos\"><a href=\"elim_coord.php?docente=".$row["docente"]."\"><img src=../theme/images/user-delete-2.ico></a></td>
//echo '<a href=\"act_mproy.php?accion=nota&docente='.$sesion.'\')"><font color=#000080>CALIFICACIONES</font></a>&nbsp;';
echo '</form>';
...
break;
		...
		{
			echo "<br>";
			//cuadro_error("NO EXISTEN PROYECTOS ASIGNADOS <b><a href=act_mproy.php  target=\"_self\">    Ir a Registrar</a></b>");	
		}
